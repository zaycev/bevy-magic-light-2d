...
fn screen_to_ndc(screen_pose: vec2<i32>, screen_size: vec2<f32>) -> vec2<f32> {
...
fn ndc_to_screen(ndc: vec2<f32>, screen_size: vec2<f32>) -> vec2<i32> {
    let screen_pose_f32 = (ndc + 1.0) / 2.0 * screen_size;
    return vec2<i32>(
...
fn screen_to_world(
    ...
    screen_size:       vec2<f32>,
    inverse_view_proj: mat4x4<f32>) -> vec2<f32> {
    return (inverse_view_proj * vec4<f32>(screen_to_ndc(screen_pose, screen_size), 0.0, 1.0)).xy;
}
